...
> 처음 숫자는 1로 시작하는게 권장 된다. 하지만 시작숫자를 자기가 정할수 있다. 예를들어 3으로 시작했으면 다음은 4, 5,... 이런식으로 간다. 이렇게 하면 줄바꿈이 자동으로 되지 않는다.
> 들여쓰기 된곳은 적용받지 않는다. (해봄)
...
## 코드블럭
> 코드블럭은 보통 스페이스 4번이나 탭으로 들여쓰기 한다. 하지만 리스트에 들어가 있으면 스페이스8번이나 탭2번을 해야한다.
...
# 이미지
> 사용방법 : `[![이미지 이름](이미지 내부URL "이미지 타이틀")](이미지 원본 웹 주소)`
